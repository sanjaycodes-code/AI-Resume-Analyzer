 ...
 */
const getPasswordResetHtml = (resetUrl: string, userName?: string): string => `
  <!DOCTYPE html>
  <html>
    <head>
      <meta charset="utf-8">
...
export const sendPasswordResetEmail = async (
      ...
      if (error) {
        ...
        if (error.message?.includes('testing emails to your own email address')) {
          console.warn(
            ...
            `To send to any recipient without a custom domain, configure Gmail SMTP (SMTP_USER & SMTP_PASS).\n`
          );
        }
  ...
  // 3. Development/Demo fallback: log the reset link prominently to console
  console.log('==================================================================');
  console.log('[EmailService: DEV / DEMO MODE — NO EMAIL CREDENTIALS CONFIGURED]');
  console.log(`Recipient: ${email}`);
  console.log(`Password Reset Link: ${resetUrl}`);
  console.log('==================================================================');
};
